...
class Document(BaseModel):
    ...
    def update_chunks(self, new_chunks: List[Chunk]) -> "Document":
        ...
        return self.model_copy(
            update={"chunks": new_chunks, "metadata": self.metadata.update_timestamp()}
        )

    def clear_content(self) -> "Document":
        """Remove all chunks from document"""
        return self.model_copy(
            update={"chunks": [], "metadata": self.metadata.update_timestamp()}
        )
...
class Library(BaseModel):
    ...
    document_ids: Set[DocumentID] = Field(
        default_factory=set, description="References to documents in this library"
    )
    metadata: Metadata = Field(default_factory=Metadata)
    index_type: str = Field(
        default="naive", description="Preferred index type for this library"
    )
    ...
    def remove_document_reference(self, document_id: DocumentID) -> "Library":
        ...
        updated_document_ids = self.document_ids.copy()
        updated_document_ids.discard(
            document_id
        )  # discard doesn't raise if not present
        return self.model_copy(
            update={
                "document_ids": updated_document_ids,
                "metadata": self.metadata.update_timestamp(),
